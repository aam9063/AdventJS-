...
function findFirstRepeated(gifts) {
    let minIndex = gifts.length; // Índice más pequeño
    let repeatedId = -1; // Número repetido

    for (let i = 0; i < gifts.length; i++) { // Recorremos la lista de regalos
        for (let j = i + 1; j < gifts.length; j++) { // Recorremos los regalos restantes
            if (gifts[i] === gifts[j] && j < minIndex) { // Si el regalo se repite y el índice es menor
                minIndex = j; // Guardamos el índice
                repeatedId = gifts[i]; // Guardamos el número repetido
            }
        }
    }

    return minIndex === gifts.length ? -1 : repeatedId; // Si no se repite ningún número, devolvemos -1 sino el número repetido
}
