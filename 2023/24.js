...
Los elfos, siempre buscando hacer ejercicio y divertirse 🏃‍♂️, deciden saltar los peldaños 
de la escalera.
...
elfos pueden saltar como máximo maxJump peldaños en un solo salto (pero pueden saltar menos 
peldaños si así lo desean).
...
function getStaircasePaths(steps, maxJump) {
    ...
    function findPaths(currentPath, remainingSteps) {
      // Si no quedan peldaños, agregamos la secuencia encontrada a la lista de resultados
      if (remainingSteps === 0) {
        result.push(currentPath);
        return;
      }
  
      // Intentar todos los saltos posibles desde 1 hasta maxJump
      for (let i = 1; i <= maxJump; i++) {
        if (remainingSteps - i >= 0) {
          findPaths([...currentPath, i], remainingSteps - i);
        }
      }
    }
  
    // Iniciar la búsqueda con un array vacío y el número total de peldaños
    findPaths([], steps);
